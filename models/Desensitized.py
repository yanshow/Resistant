...
from utils import sparse_dense_mul,drop_edge_weighted

def get_contrastive_emb_dg(logger, adj, features, adj_delete, lr, weight_decay, nb_epochs, delete_pr_weight,delete_degree_weight, beta=1,recover_percent=0.2):
    ft_size = features.shape[2]
    nb_nodes = features.shape[1]
    aug_features1 = features
    aug_features2 = features
    aug_adj1 = aug_dg_edge(adj_pre=adj,edge_weight=delete_pr_weight, adj_delete=adj_delete, recover_percent=recover_percent)  # random drop edges
    aug_adj2 = aug_dg_edge(adj_pre=adj,edge_weight=delete_degree_weight, adj_delete=adj_delete, recover_percent=recover_percent)  # random drop edges
...
def aug_dg_edge(adj_pre,edge_weight, adj_delete, recover_percent=0.2):
    recover_edge_index=drop_edge_weighted(edge_index=adj_delete, edge_weights=edge_weight, p=recover_percent)
    print("the number of recovering edges: {:04d}".format(recover_edge_index.shape[1]))
    row = recover_edge_index[0].cpu().numpy()
    col = recover_edge_index[1].cpu().numpy()
    data = np.ones_like(row, dtype=np.float32)
    recover_adj = sp.csr_matrix((data, (row, col)), shape=adj_pre.shape)
    adj_pre = sp.csr_matrix(adj_pre)
    aug_adj = recover_adj + adj_pre
    return aug_adj;

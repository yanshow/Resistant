...
class DDA(object):
    ...
    def fit(self, pretrain_ep=200, pretrain_nc=20):
        ...
        # 这是一个根据不同GNN层类型进行处理的邻接矩阵,用作节点分类网络(NC net)的输入。
        ...
        adj_t = self.adj_orig
        # 解决边预测任务中正负样本不平衡的问题。
        # norm_w是用于归一化损失函数的权重因子,pos_weight是用于调整正样本在损失函数中的贡献度。这种做法可以帮助边预测网络更好地学习边的预测模式。
        ...
        for epoch in range(self.n_epochs):
            ...
            model.train()
            # 得到vgae生成的加强融合图  和 进行过图融合的图结构放入gnn中的节点特征
            # nc_logits加强融合图
            nc_logits, adj_logits = model(adj_norm, adj_orig, features,epoch)

            # losses
            # 标签训练loss
            loss = nc_loss = nc_criterion(nc_logits[self.train_nid], labels[self.train_nid])
            # 图学习 loss 学习的图结构adj_logits 原始图adj_orig
            ...
            with torch.no_grad():
                #  只用节点分类
                nc_logits_eval = model.nc_net(adj, features)
                # nc_logits_eval, _ = model(adj_norm, adj_orig, features)
            # 节点分类正确率
            val_acc = self.eval_node_cls(nc_logits_eval[self.val_nid], labels[self.val_nid])
            # 输出边的分类效果
            adj_pred = torch.sigmoid(adj_logits.detach()).cpu()
...
class GAug_model(nn.Module):
    def __init__(self,
                 ...
                 jknet=False,
                 alpha=1,
                 sample_type='add_sample',delta=5,augment=True):
        super(GAug_model, self).__init__()
        # delta 为了graph_confidence
    ...
    def sample_adj_add_bernoulli(self, adj_logits, adj_orig, alpha,epoch):
        if torch.max(adj_logits) < 1e-6:
            edge_probs = torch.sigmoid(adj_logits)
        else:
            edge_probs = adj_logits / torch.max(adj_logits)
        # print(torch.min(edge_probs), torch.max(edge_probs))
        # edge_probs = adj_logits / torch.max(adj_logits)
        # graph confidence 越到后面 可信度越大，越来越可以值赋值更大的权重 因此最小值0.9 逐渐到 0.99  sacle_factor设定初始收敛速度 越大越容慢增大
        # graph_confidence = F.sigmoid(torch.log(torch.Tensor([epoch / scale_factor]))).item()
        # graph_confidence=round(graph_confidence,2)
        graph_confidence=round(torch.sigmoid(torch.log(torch.Tensor([epoch / self.scale_factor]))).item(),2)

        if graph_confidence<alpha:
            graph_confidence=alpha;
        # alpha=alpha*graph_confidence
        # [0.95,1]
        # 随着epoch增加，生成vgae图的可信度越来越增加，因为vgae图越来越与原图相似。因此可以增加vgae图的权重，直至0.99。

        edge_probs = graph_confidence*edge_probs + (1-graph_confidence)*adj_orig
        # sampling
        # 参数 gamble softmax
        adj_sampled = pyro.distributions.RelaxedBernoulliStraightThrough(temperature=self.temperature, probs=edge_probs).rsample()
        # making adj_sampled symmetric
    ...
    def forward(self, adj, adj_orig, features,epoch):
        adj_logits = self.ep_net(adj, features)
        if self.sample_type == 'add_sample':
            if torch.isnan(adj_logits).any() or torch.isinf(adj_logits).any():
                adj_new=adj_orig
            elif self.augment is False:
                adj_new = adj_orig
            elif self.alpha == 1:
                adj_new = self.sample_adj(adj_logits)
            else:
                # 输入位置
                adj_new = self.sample_adj_add_bernoulli(adj_logits, adj_orig, self.alpha,epoch)
        adj_new_normed = self.normalize_adj(adj_new)
        # 放入gnn模型
        nc_logits = self.nc_net(adj_new_normed, features)
        # 得到gnn模型的节点特征输出 和 增强图
